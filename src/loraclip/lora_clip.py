...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, r: int = 4):
    """Load a CLIP model
    ...
    if not jit:
        model = build_LoRA_model(state_dict or model.state_dict(), r).to(device)
        if str(device) == "cpu":
            model.float()
