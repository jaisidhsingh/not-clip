...
def prepare_split(split_name, split_info_path, class_list, image_folder, extension=".jpg"):
	...
	with open(split_info_path) as f:
		for line in tqdm(f.readlines()):
			...
			label = class_list.index(entry[0])
			path = os.path.join(image_folder, entry[0], entry[1] + extension)
...
def run_preprocess():
	tqdm.write("Starting preprocessing of Food 101 dataset ...")
	
	config = data_configs.datasets["food_101"]
