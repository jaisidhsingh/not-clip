...
def prepare_models(args):
	if args.training_method == "selective_freeze":
		...
		model.load_state_dict(torch.load('../../checkpoints/selective_freeze_pray2win.pt')["model"])
		print("Model loaded")
...
def eval(args):
	...
	default_model.eval()
	trained_model = trained_model.to(torch.float32).to(args.device)
	trained_model.eval()
	...
	prompts = [f"this is a photo of a {c}" for c in dataset.classes]
	...
	default_prompt_features /= default_prompt_features.norm(dim=-1, keepdim=True)
	trained_prompt_features /= trained_prompt_features.norm(dim=-1, keepdim=True)
	...
	for (images, labels) in loader:
		...
		default_sim = (100 * image_features.float() @ default_prompt_features.float().T).softmax(dim=-1)
		...
		bar.set_postfix({
			"default_match_accuracy": round(default_matches / total, 4),
			"trained_match_accuracy": round(trained_matches / total, 4)
		})
		bar.update(1)
	...
	# save_dir =  "/workspace/btp/results/positive_classification"
