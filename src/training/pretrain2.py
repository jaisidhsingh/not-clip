...
def prepare_model(args):
	if args.training_method == "loraclip":
		...
		retrain_words = ['not', 'without', 'presence']
...
def main(args):
	...
	optimizer = torch.optim.SGD(
		[model.token_embedding.weight], # only the embedding layer 
		lr=1e-2,
	...
	for epoch in range(args.num_epochs):
		...
		for (images, p, n1, n2, cr) in loader:
			...
			model.zero_frozen_token_grads_with_check()
				 #! just do this to make sure that everything is fine
				#! replace properly during actual training to mae things faster(?)
			# try:
			...
			if num_batches == 100:
				dump = {"model": model.state_dict(), "optimizer": optimizer.state_dict()}
				torch.save(dump, "../../checkpoints/selective_freeze_pray2win.pt")
